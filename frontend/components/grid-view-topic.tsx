...
const getTopicTransition = (index: number) => ({ delay: index * 0.05 });
...
const TopicClusterTitle = ({
  cluster,
  getDisplayLabel,
}: Readonly<Pick<TopicClusterCardProps, "cluster" | "getDisplayLabel">>) => (
...
type TopicClusterCardContentProps = TopicClusterCardProps &
  Readonly<{ imageUrl: string | null; representativeTitle: string }>;

const TopicClusterCardContent = ({
  cluster,
  index,
  isExpanded,
  getDisplayLabel,
  ...
  imageUrl,
  representativeTitle,
}: TopicClusterCardContentProps) => (
  <motion.div
    initial={TOPIC_INITIAL}
    animate={TOPIC_ANIMATE}
    transition={getTopicTransition(index)}
    data-cluster-id={cluster.cluster_id}
    className={`group relative flex cursor-pointer flex-col overflow-hidden rounded-md border bg-black/25 transition-all duration-500 hover:bg-white/[0.03] scroll-mt-6 sm:rounded-lg sm:bg-black/20 ${getTopicCardClassName(isExpanded)}`}
  >
    <button
      type="button"
    ...
    <div className="pointer-events-none relative z-10">
      <TopicClusterTitle cluster={cluster} getDisplayLabel={getDisplayLabel} />
      <TopicClusterSummary
        cluster={cluster}
...
const TopicClusterCard = (props: TopicClusterCardProps) => {
  const representative = props.cluster.representative_article;
  if (!representative) {
    return null;
  }
  return (
    <TopicClusterCardContent
      ...
      imageUrl={pickClusterImageUrl(props.cluster)}
