...
const ModalWikiBody = ({
    ...
    Pick<
      ModalWikiSheetProps,
      "tab" | "source" | "reporterName" | "articleHost" | "articleWikiContext"
    >
  >) => {
    if (tab === "source") {
  ...
  ModalWikiHeader = ({
    ...
    Pick<
      ModalWikiSheetProps,
      "tab" | "onTabChange" | "source" | "reporterName" | "hasSourceWiki" | "hasReporterWiki"
    >
  >) => (
  ...
  ModalWikiTabs = ({
    ...
    Pick<ModalWikiSheetProps, "tab" | "hasSourceWiki" | "hasReporterWiki" | "onTabChange">
  >) => {
    ...
    return (
      <div className="mt-4 flex items-center gap-2">
        ...
        <button
          type="button"
          onClick={handleReporterTabClick}
  ...
  ModalWikiTitle = ({
    ...
    source,
    reporterName,
  }: Readonly<Pick<ModalWikiSheetProps, "tab" | "source" | "reporterName">>) => (
    <div>
      <SheetTitle className="font-serif text-xl">
        {getWikiSheetTitle(tab, source, reporterName)}
      </SheetTitle>
      <SheetDescription className="mt-1 text-xs">
        Inline wiki preview from cached public-source research with direct links to the full wiki
        pages.
      </SheetDescription>
    </div>
  ),
  getWikiSheetTitle = (
    tab: ModalWikiSheetProps["tab"],
    source: string,
    reporterName: string,
  ): string => {
    if (tab === "source") {
