...
const EMPTY_COUNT = 0,
  ...
  LanguageForensicsCard = ({
    ...
    loading,
    error,
  }: Readonly<LanguageForensicsCardProps>) => {
    ...
    examples.length > EMPTY_COUNT && (
      <div className="mt-4 space-y-2">
        {examples.map((example) => (
          <LanguageExampleCard key={example.sentence} example={example} />
        ))}
      </div>
    ),
  LanguageForensicsHeader = ({
    status,
    loading,
  }: Readonly<{ status: LanguageStatus; loading: boolean }>) => (
  ...
  LanguageForensicsMetrics = ({
    ...
    metrics: readonly Readonly<{
      key: LanguageMetricKey;
      label: string;
      metric: LanguageDiagnosticMetric | undefined;
    }>[];
  }>) => (
  ...
  LanguageForensicsSummary = ({
    summary,
    loading,
  }: Readonly<{ summary?: string; loading: boolean }>) => {
    ...
    return (
      <p className="mt-4 text-sm text-muted-foreground">
        No diagnostic result available for this article.
      </p>
    );
  },
  ...
  getExampleLabel = (example: Readonly<LanguageDiagnosticExample>): string => {
    ...
    if (isNonEmptyString(example.pattern)) {
      return example.pattern;
    }
    return "example";
  },
  getLanguageExamples = (diagnostics: Readonly<LanguageDiagnostics> | null | undefined) =>
    LANGUAGE_EXAMPLE_KEYS.flatMap((key) => diagnostics?.[key]?.examples ?? []).slice(
      EMPTY_COUNT,
      MAX_LANGUAGE_EXAMPLES,
    ),
