...
const BrowserTelemetry = (): undefined => {
  ...
  createWindowErrorHandler =
    (enqueue: (event: Readonly<BrowserEvidence>) => void): ((event: BrowserErrorEvent) => void) =>
    (event) => {
  ...
  getResourceSource = (target: Readonly<EventTarget> | null): ResourceSource | undefined => {
    if (!(target instanceof HTMLElement)) {
      return void 0;
    }
    const tagName = target.tagName.toLowerCase();
    let source =
      (target.getAttribute("src") ?? target.getAttribute("href") ?? tagName).split("?")[ZERO] ??
      tagName;
