...
const EMPTY_COUNT = 0,
  ...
  getArticleHost = (url?: string): string | undefined => {
    if (url === undefined || !isExtractableUrl(url)) {return undefined}
  ...
  getArticleTextForMetrics = (
    ...
    if (primary !== undefined && primary !== "") {return primary.trim()}
    return (summary ?? "").trim()
  },
  ...
  getArticleWordMetrics = (text: string) => {
    ...
    if (text !== "") {wordCount = text.split(/\s+/u).filter(Boolean).length}
    return { estimatedReadMinutes: Math.max(MINIMUM_READING_MINUTES, Math.ceil(wordCount / WORDS_PER_READING_MINUTE)), wordCount }
  },

  getHighlightSyncFailureStatus = (): ModalHighlightSyncStatus => {
    if (getOfflineSyncStatus() === "offline") {return "offline"}
    return "failed"
  },

  getModalReaderText = (fullArticleText: string | undefined, articleSummary: string): string => {
    if (isNonEmptyString(fullArticleText)) {return fullArticleText}
    if (articleSummary !== "") {return articleSummary}
    return ""
  },

  getModalReaderTitleClassName = (isExpanded: boolean): string => {
    if (isExpanded) {return "mb-6 font-bold font-serif text-3xl text-foreground"}
    return "mb-6 font-bold font-serif text-xl text-foreground"
  },

  getOfflineSyncStatus = (): ModalHighlightSyncStatus => {
    if (globalThis.navigator.onLine) {return "idle"}
    return "offline"
  },

  getReaderContentClassName = (isExpanded: boolean): string => {
    if (isExpanded) {return "reading-prose space-y-6"}
  ...
  getRenderedLanguageDiagnostics = (
    ...
    if (aiAnalysis?.language_diagnostics?.success === true) {return aiAnalysis.language_diagnostics}
  ...
  getVisibleReaderHighlights = (
    ...
    if (showHighlights) {return visibleHighlights}
    return []
  },
  ...
  processHighlightItems = async (
    ...
    const processNext = async (
      ...
      if (controller.latestSyncToken.current !== syncToken || index >= actionable.length) {return}
  ...
  renderReaderContent = (props: Readonly<ReaderProps>) => {
    if (props.articleLoading && isNonEmptyString(props.fullArticleText)) {
      return <><ModalArticleReaderLoading updating />{renderReaderBodyContent(props)}</>
    }
    if (props.articleLoading) {return <ModalArticleReaderLoading updating={false} />}
  ...
  syncDeletedHighlight = async (
    ...
    if (id !== undefined) {await services.deleteHighlight(id)}
  ...
  syncOneHighlight = (
    ...
    if (item.pending_op === "create") {return syncCreatedHighlight(item, current, services)}
    if (item.pending_op === "update") {return syncUpdatedHighlight(item, current, services)}
    if (item.pending_op === "delete") {return syncDeletedHighlight(item, current, services)}
