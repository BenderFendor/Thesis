...
import { ModalHeroSourceLinks, ModalHeroVisual } from "./article-detail-modal-hero-parts";
...
const EMPTY_COUNT = 0,
  ...
  ModalArticleNavigation = ({
    ...
    const handleNavigationClick = useCallback(
      (event: NavigationClickEvent): void => {
        const { direction } = event.currentTarget.dataset;
        if (direction === "prev" || direction === "next") {
          onArticleNavigate(direction);
        }
      },
      [onArticleNavigate],
    );
  ...
  ModalHeroAuthors = ({
    article,
    reporterName,
  }: Readonly<{ article: NewsArticle; reporterName: string }>) => {
    if (
      reporterName !== "" ||
      article.authors === undefined ||
      article.authors.length === EMPTY_COUNT
    ) {
      return false;
    }
    return (
      <>
        ...
        <span className="text-xs text-foreground/80">
          {article.authors.slice(EMPTY_COUNT, MAX_AUTHOR_NAMES).join(", ")}
        </span>
      </>
    );
  },
  ...
  ModalHeroReporter = ({
    ...
    const handleCloseLinkClick = useCallback(
      ...
      handleReporterClick = useCallback(
        (event: PropagationClickEvent): void => {
          event.stopPropagation();
          onOpenReporterWiki();
        },
  ...
  ModalSummaryQuote = ({
    summary,
    isExpanded,
  }: Readonly<{ summary: string; isExpanded: boolean }>) => (
    <div className={getSummaryClassName(isExpanded)}>
      <p
        className={`text-foreground/80 leading-relaxed italic ${getSummaryTextClassName(isExpanded)}`}
      >
        {summary}
      </p>
    </div>
  ...
  getModalHeroClassName = (isExpanded: boolean, hasImage: boolean): string => {
    ...
    if (!hasImage) {
      classes.push("editorial-modal-fallback");
    }
    return classes.join(" ");
  },
  ...
  getModalHeroTitleClassName = (isExpanded: boolean): string => {
    if (isExpanded) {
      return "mb-6 font-serif text-4xl leading-tight text-foreground md:text-6xl";
    }
  ...
  getSummaryClassName = (isExpanded: boolean): string => {
    if (isExpanded) {
      return "mb-12 border-l-4 border-primary bg-card/50 px-6 py-4";
    }
...
export { ModalHeaderControls, ModalHero, ModalProgressRail, ModalSummaryQuote };
