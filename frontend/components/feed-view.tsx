...
import { Heart, Bookmark, ExternalLink, Star, ChevronDown, ChevronUp } from "lucide-react";
...
export function FeedView({ articles: propArticles, loading }: FeedViewProps) {
  ...
  const scrollToNext = useCallback(() => {
    ...
    const nextElement = container.querySelector(`[data-index="${activeIndex + 1}"]`);
  ...
  const scrollToPrev = useCallback(() => {
    ...
    const prevElement = container.querySelector(`[data-index="${activeIndex - 1}"]`);
    ...
    return (
      <div className="flex-1 h-full w-full flex items-center justify-center bg-[var(--news-bg-primary)]">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-muted-foreground">Indexing articles...</span>
        </div>
      </div>
    );
  }
  ...
  if (articles.length === 0) {
    return (
      <div className="flex-1 h-full w-full flex items-center justify-center bg-[var(--news-bg-primary)]">
        <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-muted-foreground">No coverage found for this category.</span>
      </div>
    );
  }
          ...
          >
            ...
            <div className="relative z-10 h-full flex flex-col justify-end p-6 md:p-10 lg:p-12">
              <div className="absolute top-6 left-6 md:top-8 md:left-8 flex flex-wrap items-center gap-3">
                ...
                <Badge variant="outline" className="text-[10px] font-mono uppercase tracking-[0.2em] border-white/10 bg-black/40 backdrop-blur-sm text-white/70 px-3 py-1">
              ...
              <div className="flex flex-col md:flex-row md:items-end gap-6 md:gap-10">
                <div className="flex-1 space-y-4">
                  ...
                  <div className="flex flex-wrap items-center gap-4 pt-2">
                    ...
                    >
                      <Button size="sm" variant="outline" className="bg-white/5 text-white border-white/20 hover:bg-white/10 font-mono text-[10px] uppercase tracking-[0.3em]">
                ...
                <div className="flex flex-row md:flex-col items-center gap-2 md:gap-4 md:bg-black/20 md:backdrop-blur-xl md:p-3 md:border md:border-white/10 md:rounded-full">
                  ...
                  >
                    <Heart className={cn("w-6 h-6 transition-all", likedIds.has(article.id) ? "fill-primary text-primary scale-110" : "text-white/60")} />
                  </Button>
                  ...
                  >
                    <Star className={cn("w-6 h-6 transition-all", isFavorite(article.sourceId) ? "fill-amber-400 text-amber-400 scale-110" : "text-white/60")} />
                  </Button>
                  ...
                  >
                    <Bookmark className={cn("w-6 h-6 transition-all", bookmarkIds.has(article.id) ? "fill-white text-white scale-110" : "text-white/60")} />
                  </Button>
                </div>
              </div>
            </div>
