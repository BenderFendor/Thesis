...
import type { Message, ReadonlyThinkingStep } from "../model/types";
...
const createStepEntries = (steps: readonly ReadonlyThinkingStep[]) => {
  const occurrences = new Map<string, number>();
  return steps.map((step) => {
    const identity = `${step.type}-${step.timestamp}-${step.content}`;
    const occurrence = occurrences.get(identity) ?? 0;
    occurrences.set(identity, occurrence + 1);
    return { key: `${identity}-${occurrence}`, step };
  });
};
...
const ThinkingSteps = ({
  ...
  const entries = createStepEntries(steps);
  return (
    <div className="mt-3 space-y-2">
      {entries.map(({ key, step }, stepIndex) => (
        <details
          key={`${message.id}-${key}`}
          className="rounded-2xl border border-border/20 bg-background/40 p-3"
        >
          <summary className="cursor-pointer font-mono text-xs uppercase tracking-wide text-muted-foreground/70">
            Step {stepIndex + 1}: {step.type.replace("_", " ")}
          </summary>
          <p className="mt-2 max-h-64 overflow-y-auto whitespace-pre-wrap text-xs text-muted-foreground [overflow-wrap:anywhere]">{step.content}</p>
        </details>
      ))}
    </div>
  );
