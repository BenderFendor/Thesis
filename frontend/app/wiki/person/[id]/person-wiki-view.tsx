...
const PersonWikiView = ({ entityId }: Readonly<{ entityId: string }>) => {
    ...
    <div className="flex bg-background min-h-screen text-foreground overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto relative z-10 custom-scrollbar">
        ...
        <main className="mx-auto grid max-w-[1500px] gap-5 p-4 lg:grid-cols-[300px_minmax(0,1fr)]">
          <PersonWikiSidebar data={data} externalIds={externalIds} roleBreakdown={roleBreakdown} />

          <PersonWikiPanels chain={chain} controls={controls} data={data} />
        </main>
      </div>
    </div>
  );
...
interface PersonWikiPanelsProps {
  readonly chain: ReturnType<typeof parseOwnershipChain>;
  readonly controls: ReturnType<typeof parseControls>;
  readonly data: DeepReadonly<AtlasEntityRecord>;
}

const PersonWikiPanels = ({ chain, controls, data }: DeepReadonly<PersonWikiPanelsProps>) => (
  <section className="space-y-5">
    ...
    >
      {(() => {
  if (chain.length > 1) {
    return <div className="rounded-2xl border border-white/5 bg-black/20 p-5">
  ...
  if (controls.length > 0) {
    return <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {controls.map(entry => {
        const card = <div className="group rounded-2xl border border-white/5 bg-black/20 transition-all hover:bg-white/[0.03] hover:-translate-y-px hover:shadow-lg p-4 relative overflow-hidden">
              ...
              return <span className="font-mono text-[10px] tracking-widest text-muted-foreground">
        ...
        if (hasText(entry.profile_path)) {
          return <Link key={entry.entity_id} href={entry.profile_path}>
                {card}
              </Link>;
        }
        return <div key={entry.entity_id}>{card}</div>;
      })}
        </div>;
  }
  return <p className="text-[10px] font-mono tracking-widest uppercase text-muted-foreground">
  ...
  if (data.connections.length > 0) {
    return data.connections.map(({
      ...
      const row = <div className="flex items-center justify-between gap-3 rounded-xl border border-white/5 bg-black/20 px-3 py-2 text-sm transition-all hover:bg-white/[0.03] hover:-translate-y-px hover:shadow-lg group">
      ...
      if (hasText(entity.profile_path)) {
        return <Link key={edge.id} href={entity.profile_path}>
                {row}
              </Link>;
      }
      return <div key={edge.id}>{row}</div>;
    });
  }
  return <p className="text-[10px] font-mono tracking-widest uppercase text-muted-foreground">
  ...
  if (data.evidence.length > 0) {
    return <div className="space-y-3">
          {data.evidence.map(item => <div key={item.id} className="rounded-2xl border border-white/5 bg-black/20 p-4 transition-all hover:bg-white/[0.03] hover:-translate-y-px hover:shadow-lg">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="text-sm font-serif">{item.source_name ?? item.source_type}</div>
                  {(() => {
              if (hasText(item.excerpt)) {
                return <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
                      {item.excerpt}
                    </p>;
              }
              return null;
                  ...
                  <div className="mt-2 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
                    {(() => {
                if (hasText(item.retrieved_at)) {
                  return formatArticleDate(item.retrieved_at);
                }
                return "Not recorded";
              })()}
                  </div>
                ...
                {(() => {
            if (hasText(item.source_url)) {
              return <a href={item.source_url} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-white transition-colors shrink-0">
  ...
  return <div className="flex items-center gap-2 text-[10px] font-mono tracking-widest uppercase text-muted-foreground">
...
interface PersonWikiSidebarProps {
  readonly data: DeepReadonly<AtlasEntityRecord>;
  readonly externalIds: ReturnType<typeof parseExternalIds>;
  readonly roleBreakdown: ReturnType<typeof parseRoleBreakdown>;
}

const PersonWikiSidebar = ({
  data,
  externalIds,
  roleBreakdown,
}: DeepReadonly<PersonWikiSidebarProps>) => (
  <aside className="rounded-2xl border bg-black/40 backdrop-blur-2xl border-white/10 p-4 shadow-[0_8px_32px_rgba(0,0,0,0.4)] ring-1 ring-white/5 lg:sticky lg:top-4 lg:h-[calc(100vh-2rem)] lg:overflow-y-auto custom-scrollbar">
    ...
    <SidebarCard title="Quick Facts">
      ...
      {Boolean(data.last_verified_at) && <SidebarFact label="Last verified" value={formatArticleDate(data.last_verified_at)} />}
    </SidebarCard>

    {Object.keys(roleBreakdown).length > 0 && <SidebarCard title="Role Breakdown">
        <div className="space-y-2">
          {Object.entries(roleBreakdown).map(([role, count]) => <div key={role} className="flex items-center justify-between text-[10px] font-mono tracking-widest uppercase">
  ...
  if (externalIds.length > 0) {
    return <div className="space-y-2 text-sm">
          {externalIds.map(extId => <div key={`${extId.scheme}-${extId.value}`} className="flex items-center gap-2">
              {(() => {
          if (hasText(extId.url)) {
            return <a href={extId.url} target="_blank" rel="noreferrer" className="group flex items-center gap-2 text-muted-foreground transition-colors hover:text-white">
          ...
          return <span className="truncate font-mono text-[10px] tracking-widest uppercase text-muted-foreground">
                  {extId.scheme.replaceAll("_", " ")}: {extId.value}
                </span>;
        })()}
            </div>)}
