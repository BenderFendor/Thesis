...
interface PersonWikiPanelsProps {
  ...
  readonly data: Readonly<AtlasEntityRecord>;
}

function PersonWikiPanels({
  chain,
  controls,
  data,
}: Readonly<PersonWikiPanelsProps>) {
  return (
    <section className="space-y-5">
      <Panel title="Ownership Chain" eyebrow="Entities this person owns equity in, up through any further owner">
        {chain.length > 1 ? (
          <div className="rounded-2xl border border-white/5 bg-black/20 p-5">
            <OwnershipChain chain={chain} currentEntityId={data.id} />
          </div>
        ) : (
          <p className="text-[10px] font-mono tracking-widest uppercase text-muted-foreground">No accepted ownership chain recorded above this person.</p>
        )}
      </Panel>

      <Panel title="Controls" eyebrow="Everything this person reaches through accepted ownership/control edges">
        {controls.length > 0 ? (
          ...
          <p className="text-[10px] font-mono tracking-widest uppercase text-muted-foreground">No downstream entities recorded under this person.</p>
        )}
      </Panel>
...
function PersonWikiSidebar({
  data,
  externalIds,
  roleBreakdown,
}: Readonly<PersonWikiSidebarProps>) {
  return (
