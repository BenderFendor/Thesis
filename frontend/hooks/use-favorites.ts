...
const EMPTY_FAVORITE_IDS: string[] = [],
  StringArraySchema = createStorageSchema(z.array(z.string())),
  toggleFavoriteId = (favoriteIds: readonly string[], sourceId: string): void => {
  ...
  useFavorites = () => {
    const clearFavorites = useCallback(() => {
      ...
      favoriteIds = useSyncExternalStore(
        (onChange) => subscribeToStorageKey(STORAGE_KEYS.FAVORITE_SOURCES, onChange),
        () =>
          getStorageSnapshot(
      ...
      isFavorite = useCallback((sourceId: string): boolean => favorites.has(sourceId), [favorites]),
