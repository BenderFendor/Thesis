...
type AtlasLayoutRequest =
  ConstructorParameters<typeof AtlasForceLayoutRunner> extends readonly [infer Request]
    ? Request
    : never;
...
const CENTER_DIVISOR = 2,
  ...
  advanceRunner = (runner: Readonly<AtlasForceLayoutRunner>): void => {
    for (
      let step = ZERO_COORDINATE;
      step < ITERATIONS_PER_FRAME && runner.hasNext();
      step += INDEX_STEP
    ) {
      runner.step();
    }
  ...
  useAtlasLayout = (options: UseAtlasLayoutOptions): LayoutState => {
    const cacheKey = topologyKey(options),
      ...
      empty =
        options.nodes.length === ZERO_COORDINATE ||
        options.width <= ZERO_COORDINATE ||
        options.height <= ZERO_COORDINATE,
