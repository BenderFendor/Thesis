...
const useAtlasStageControllers = (
  graph: AtlasStageShellProps["graph"],
  onSelect: AtlasSelectionHandler,
) => {
  const edges = graph?.edges ?? EMPTY_EDGES;
  const nodes = graph?.nodes ?? EMPTY_NODES;
  const nodesById = useMemo(() => new Map(nodes.map((node) => [node.id, node])), [nodes]);
  const selectDockNode = useCallback<ReadonlyButtonClickHandler>(
    (event) => {
  ...
  return { edges, nodes, selectDockNode, selectGraphNode };
};

const AtlasStageShell = (props: Readonly<AtlasStageShellProps>) => {
    const { graph, onSelect, state } = props;
    const { edges, nodes, selectDockNode, selectGraphNode } = useAtlasStageControllers(
      graph,
      onSelect,
    );
      ...
      <>
        <div className={styles.stage}>
          <AtlasContextPanel
            ...
            ownershipCoverage={props.ownershipCoverage}
          />
          <AtlasToolbar
            state={state}
            selectedNode={props.selectedNode}
  ...
  EntityFilters = (props: Readonly<EntityFiltersProps>) => {
    const updateEntities = useCallback<ReadonlyButtonClickHandler>(
      (event) => {
    ...
    <>
      {props.graph?.truncated === true && (
        <output className={styles.warningBanner}>
          ...
          This is a bounded graph. Hidden entities or edges reached the{" "}
          {props.graph.truncation_reason?.replaceAll("_", " ")}.
          <button type="button" className={styles.pillButton} onClick={props.onOpenIndex}>
            Browse full index
          </button>
        </output>
      )}
      {props.error !== null && (
        <div className={styles.errorBanner} role="alert">
          ...
          <button type="button" className={styles.pillButton} onClick={props.onRetry}>
            Retry
          </button>
        </div>
      )}
  ...
  LayoutControl = (props: Readonly<LayoutControlProps>) => {
    const handleChange = useCallback<ReadonlySelectChangeHandler>(
      (event) => {
  ...
  RecordCard = (props: Readonly<RecordCardProps>) => (
    ...
    >
      <span className={styles.entityMark} data-type={props.node.entity_type} aria-hidden="true">
        {props.node.entity_type
          .slice(EMPTY_COLLECTION_SIZE, NODE_LABEL_PREFIX_LENGTH)
          .toUpperCase()}
      </span>
  ...
  RecordCardText = (props: Readonly<RecordCardTextProps>) => (
    <span className="min-w-0">
      ...
      <span className="mt-1 block truncate font-mono text-[9px] uppercase tracking-[0.13em] text-[#77736a]">
        {props.node.connection_count} links ·{" "}
        {props.node.status ?? props.node.confidence_tier ?? "unresolved"}
      </span>
    </span>
  ),
    ...
    <footer className={styles.dock}>
      <div>
        ...
        <div className="mt-1 text-xs text-[#77736a]">
          Selected, recent, and high-salience visible entities
        </div>
      </div>
      <div className={styles.dockList}>
        {props.dockNodes.map((node) => (
          <RecordCard
            ...
            selected={props.state.selected === node.id}
          />
        ))}
      </div>
      <div className="flex items-center gap-2">
        <DockActionButton action="browse" label="Browse all" onClick={props.onOpenIndex} />
        <DockActionButton action="operations" label="Operations" onClick={props.onOpenOperations} />
      </div>
    </footer>
  ),
  RelationFilters = (props: Readonly<RelationFiltersProps>) => {
    const updateRelations = useCallback<ReadonlyButtonClickHandler>(
      (event) => {
        const { currentTarget } = event,
  ...
  toggleValue = <Value extends string>(
