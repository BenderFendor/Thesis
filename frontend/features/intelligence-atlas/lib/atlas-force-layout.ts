...
type AtlasLayoutNodeInput = Readonly<
  Pick<AtlasNode, "id" | "entity_type" | "country_code" | "connection_count">
>;
type AtlasLayoutEdgeInput = Readonly<
  Pick<AtlasEdge, "source_id" | "target_id" | "relation_type" | "weight">
>;
...
const CANVAS_PADDING = 48,
  ...
  edgeDistance = (edge: AtlasLayoutEdgeInput): number => {
    if (edge.relation_type === "ownership") {
      return OWNERSHIP_EDGE_DISTANCE;
    }
  ...
  groupedInitialPosition = (
    ...
    const centerAngle =
        (hashValue(groupKey(node, request.layout)) % DEGREES_PER_CIRCLE) *
  ...
  groupingRadius = (layout: AtlasLayoutMode): number => {
    if (layout === "geography") {
      return GEOGRAPHY_GROUPING_RADIUS;
    }
...
class AtlasForceLayoutRunner {
  ...
  constructor(request: AtlasLayoutRequest) {
    ...
    const proposedIterations =
      ITERATION_BASE - Math.floor(request.nodes.length / NODES_PER_ITERATION_STEP);
  ...
  private applyRepulsion(
    ...
    for (
      let leftIndex = ZERO_COORDINATE;
      leftIndex < this.request.nodes.length;
      leftIndex += INDEX_STEP
    ) {
  ...
  private applyRepulsionFromNode(
    ...
    for (
      let rightIndex = leftIndex + INDEX_STEP;
      rightIndex < this.request.nodes.length;
      rightIndex += INDEX_STEP
    ) {
  ...
  private applySpring(
    ...
    }
    {
      const distance = Math.max(
        ...
        horizontalDelta =
          (targetPosition.getHorizontal() - sourcePosition.getHorizontal()) / distance,
        spring =
  ...
  private groupTarget(node: AtlasLayoutNodeInput): Readonly<PositionVector> {
    const angle =
        (hashValue(groupKey(node, this.request.layout)) % DEGREES_PER_CIRCLE) *
        (Math.PI / (DEGREES_PER_CIRCLE / CENTER_DIVISOR)),
  ...
  private radialTarget(node: AtlasLayoutNodeInput): Readonly<PositionVector> {
    ...
    if (this.request.selectedId === null) {
      return this.groupTarget(node);
    }
    let radius = OTHER_NODE_RADIUS;
    if (directNeighbor) {
      radius = DIRECT_NEIGHBOR_RADIUS;
    }
    if (node.id === this.request.selectedId) {
      radius = ZERO_COORDINATE;
    }
    return createVector(
  ...
  private ownershipTarget(
    node: AtlasLayoutNodeInput,
    fallback: Readonly<PositionVector>,
  ): Readonly<PositionVector> {
    if (node.entity_type === "organization") {
      return createVector(
        this.request.width * ORGANIZATION_X_RATIO,
        this.request.height * ORGANIZATION_Y_RATIO,
      );
    }
    ...
    if (node.entity_type === "person") {
      return createVector(
        this.request.width * PERSON_X_RATIO,
        this.request.height * PERSON_Y_RATIO,
      );
    }
    return fallback;
  }
  ...
  private targetForNode(node: AtlasLayoutNodeInput): Readonly<PositionVector> {
    ...
    if (this.request.layout === "radial") {
      return this.radialTarget(node);
    }
    if (this.request.layout === "ownership") {
      return this.ownershipTarget(node, grouped);
    }
    return grouped;
  }
  ...
  private updateNode(
    ...
    position.set(
      clampCoordinate(
        position.getHorizontal() + velocity.getHorizontal(),
        this.request.width,
        this.padding,
      ),
      clampCoordinate(
        position.getVertical() + velocity.getVertical(),
        this.request.height,
        this.padding,
      ),
    );
  ...
  step(): void {
    const alpha = INDEX_STEP - this.iteration / this.totalIterations,
      forces = new Map(
        this.request.nodes.map((node) => [node.id, createVector(ZERO_COORDINATE, ZERO_COORDINATE)]),
      );
  ...
  getPositions() {
    return Object.fromEntries(
      [...this.positions.entries()].map(
        ([nodeId, position]: readonly [string, PositionVector]): readonly [
          string,
          AtlasPosition,
        ] => [nodeId, createPosition(position.getHorizontal(), position.getVertical())],
      ),
    );
  }
}

export { AtlasForceLayoutRunner, type AtlasPosition };
