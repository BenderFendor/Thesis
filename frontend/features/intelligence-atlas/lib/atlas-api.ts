...
const DEFAULT_INDEX_LIMIT = 60,
  ...
  atlasGraphQueryString = (filters: AtlasGraphFilterInput): string => {
    ...
    for (const [key, value] of createListQueryEntries(listFilters)) {
      params.append(key, value);
    }
    for (const [key, value] of createGraphOptionalQueryEntries(filters)) {
  ...
  buildAtlasIndexQuery = (params: AtlasIndexParams): string => {
    ...
    for (const [key, value] of createOptionalQueryEntries(optionalTextFilters)) {
      query.set(key, value);
    }
    query.set("limit", String(params.limit ?? DEFAULT_INDEX_LIMIT));
    return query.toString();
  },
  createGraphOptionalQueryEntries = (
    ...
    if (filters.min_confidence > MINIMUM_FILTER_VALUE) {
      optionalTextFilters.push(["min_confidence", String(filters.min_confidence)]);
    }
    if (filters.neighbors > MINIMUM_FILTER_VALUE) {
      optionalTextFilters.push(["neighbors", String(filters.neighbors)]);
    }
    if (filters.accepted_only === true) {
      optionalTextFilters.push(["accepted_only", "true"]);
    }
  ...
  createListQueryEntries = (
    ...
    listFilters.flatMap(([key, values]): (readonly [string, string])[] => {
      if (values.length <= EMPTY_VALUE_LENGTH) {
        return [];
      }
  ...
  createOptionalQueryEntries = (
    ...
    for (const [key, value] of optionalTextFilters) {
      if (value !== null && value !== undefined && value.length > EMPTY_VALUE_LENGTH) {
  ...
  exportAtlas = async (
    ...
    if (!response.ok) {
      throw new Error(`Atlas export failed with status ${response.status}`);
    }
    {
      const anchor = document.createElement("a"),
        ...
        filename =
          /filename="?(?<filename>[^";]+)"?/iu.exec(disposition)?.groups?.filename ??
  ...
  fetchAtlasEntity = async (
    entityId: string,
    signal?: RequestSignal,
  ): Promise<AtlasEntityRecord> => {
    const response = await fetch(
      `${API_BASE_URL}/api/wiki/atlas/entities/${encodeURIComponent(entityId)}`,
      { signal },
    );
    return parseResponse<AtlasEntityRecord>(response, AtlasEntityRecordSchema);
  },
  fetchAtlasGraph = async (
    filters: AtlasGraphFilterInput,
    signal?: RequestSignal,
  ): Promise<AtlasGraphResponse> => {
  ...
  fetchFundingBiasAnalysis = async (
    signal?: RequestSignal,
  ): Promise<FundingBiasAnalysisResponse> => {
    const response = await fetch(`${API_BASE_URL}/api/wiki/atlas/analysis/funding-bias`, {
      signal,
    });
    return parseResponse<FundingBiasAnalysisResponse>(response, FundingBiasAnalysisResponseSchema);
  },
  fetchMediaMeasurements = async (
    sourceName: string,
    signal?: RequestSignal,
  ): Promise<AtlasMeasurementsResponse> => {
    const query = new URLSearchParams({ source_name: sourceName }),
      response = await fetch(
        `${API_BASE_URL}/api/wiki/atlas/analysis/media-measurements?${query}`,
        { signal },
      );
