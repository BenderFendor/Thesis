...
const humanize = (value: Readonly<string>): string =>
  value.replaceAll("_", " ").replaceAll(/\b\w/gu, (letter) => letter.toUpperCase());

interface AtlasMetricsProps {
...
const AtlasMetrics = ({
  ...
  totalStats,
  ownershipCoverage,
}: Readonly<AtlasMetricsProps>) => (
  <div className={styles.metrics}>
    <Metric
      label="Outlets"
      value={resolveEntityCount(currentStats, "visible_outlets", totalStats, "total_outlets")}
    />
    <Metric
      label="Organizations"
      value={resolveEntityCount(
        currentStats,
        "visible_organizations",
        totalStats,
        "total_organizations",
    ...
    <Metric
      label="People"
      value={resolveEntityCount(currentStats, "visible_people", totalStats, "total_people")}
    />
    <Metric
      label="Reporters"
      value={resolveEntityCount(
...
const AtlasContextPanel = ({
  ...
  const title =
    selectedNode?.label ?? "Follow publication, ownership, reporter, and evidence relationships.";
  let copy =
