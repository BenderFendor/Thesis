...
import { fetchArticleContentText } from "./article-content";
...
const DEFAULT_ARTICLE_DETAIL_SERVICES: ArticleDetailServices = {
  ...
  fetchFullArticleText = async (
    ...
    return requestFullArticleText(article.url, initialText, signal);
  },
  ...
  getInitialArticleText = (
    article: Readonly<Pick<NewsArticle, "content" | "hasFullContent">>,
  ): string | undefined => {
  ...
  loadModalHighlights = ({
    ...
    services,
    setHighlights,
  }: Readonly<ModalHighlightLoaderProps>): void => {
    const store = loadHighlightStore(articleUrl);
    setHighlights(store.highlights);
    logHighlightDebug(debugEnabled, "[Highlights] loaded local store", {
      count: store.highlights.length,
      url: articleUrl,
    });
    void loadRemoteHighlights({
      articleUrl,
      debugEnabled,
      localStore: store,
      services,
  ...
  loadRemoteHighlights = async ({
    ...
    try {
      ...
      logHighlightDebug(debugEnabled, "[Highlights] fetch failed", {
        error: errorMessage,
  ...
  logHighlightMerge = (
    ...
    logHighlightDebug(enabled, "[Highlights] fetched server highlights", {
      count: serverCount,
      url: articleUrl,
    });
    logHighlightDebug(enabled, "[Highlights] merged highlights", {
      count: mergedCount,
      url: articleUrl,
    });
  },
  requestFullArticleText = async (
    ...
    try {
      const extractedText = await fetchArticleContentText(articleUrl, signal);
...
export { buildObsidianMarkdown, highlightStableId } from "./highlight-utils";
