...
import { OgImageResponseSchema } from "./response-schemas";
...
function cacheOgImage(url: string, imageUrl: string | null, ttlMs: number): string | null {
...
export async function fetchOGImage(url: string): Promise<string | null> {
  ...
  const pending = (async (): Promise<string | null> => {
    try {
      ...
      return cacheOgImage(url, result.image_url, SUCCESS_TTL_MS);
    } catch {
      try {
