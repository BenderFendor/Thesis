...
describe("computePolygonHeatFast", () => {
  ...
  it("returns zero heat when the country has no display count", () => {
    const heat = computePolygonHeatFast(baseFeature, { ...context, displayCounts: { CA: 1 }, maxCount: 1, maxMentionCount: 1, mentionCounts: { CA: 1 } });
    expect(heat).not.toBeNull();
    if (heat === null) {throw new Error("expected heat");}
  ...
  it("scales heat by the max count", () => {
    ...
    const covered = computePolygonHeatFast(baseFeature, { ...context, displayCounts: { US: 5 }, maxCount: 5 });
