...
describe("highlightUtils", () => {
  it("renders and activates a highlight with mouse and keyboard input", () => {
    ...
    const onHighlightClick =
      jest.fn<(stableId: string, element: Readonly<HighlightAnchorElement>) => void>();

    render(
      <div>{renderHighlightedContent("Read hello world", [HIGHLIGHT], onHighlightClick)}</div>,
    );
  ...
  it("keeps markdown export and DOM offsets aligned with the article text", () => {
    ...
    expect(getGlobalOffset(root, textNode, HIGHLIGHT.character_start)).toBe(
      HIGHLIGHT.character_start,
    );
