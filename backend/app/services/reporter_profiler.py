...
class ReporterProfiler:
    ...
    async def profile_reporter(
        ...
        if self.client:
            profile = await self._ai_enhance_profile(profile, organization, article_context)
    ...
    def _normalize_name(self, name: str) -> str:
        ...
        name = re.sub(r'\b(Dr|Mr|Mrs|Ms|Jr|Sr|III|II|IV)\b\.?', '', name, flags=re.IGNORECASE)
        # Lowercase and strip
        return name.lower().strip()
    
    async def _search_wikipedia(self, name: str, organization: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _merge_profile_data(
        ...
        if wikipedia:
            ...
            profile["research_confidence"] = "high" if wikipedia.get("confidence") == "high" else "medium"
...
{{
  ...
  "assessment_notes": "Brief explanation"
            ...
            if json_match:
                ...
                if ai_data.get("political_leaning") and not profile.get("political_leaning"):
                    profile["political_leaning"] = ai_data["political_leaning"]
                    profile["leaning_confidence"] = ai_data.get("leaning_confidence", "low")
