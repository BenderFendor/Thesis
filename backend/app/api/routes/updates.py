...
_update_subscribers: list[asyncio.Queue[UpdateEvent]] = []
_event_counter = 0


def _remove_subscriber(queue: asyncio.Queue[UpdateEvent]) -> None:
    try:
...
async def broadcast_update(event_type: str, data: Dict[str, Any] | None = None) -> None:
    ...
    dead_queues: list[asyncio.Queue[UpdateEvent]] = []
