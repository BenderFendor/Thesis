...
async def test_postgresql():
    ...
    try:
        from app.database import engine, AsyncSessionLocal, Article
...
def test_chromadb():
    ...
    try:
        ...
        print(f"✅ ChromaDB connected: {stats['total_articles']} documents in '{stats['collection_name']}'")
...
async def test_dual_write():
    ...
    try:
        ...
        async with AsyncSessionLocal() as session:
            ...
            await session.commit()
            print(f"✅ Article updated with chroma_id reference")
        
        # 4. Test semantic search
        results = vector_store.search_similar(
            query="test article database integration",
            limit=5
        )
        
        found = any(r['chroma_id'] == chroma_id for r in results)
        if found:
            print(f"✅ Test article found in semantic search")
        else:
            print(f"⚠️ Test article not found in search (may need more data)")
